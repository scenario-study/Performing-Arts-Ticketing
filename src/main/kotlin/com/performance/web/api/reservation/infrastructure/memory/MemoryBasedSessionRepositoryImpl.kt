...
class MemoryBasedSessionRepositoryImpl : SessionRepository {
    ...
    private final val sessionStore = ConcurrentHashMap<Long, Session>(
        mapOf(
            1L to Session(
                id = 1L,
    ...
    private final val seatStores = ConcurrentHashMap<Long, List<Seat>>(
        mapOf(
            1L to listOf(
                Seat(
                    id = 1L,
    ...
    override fun save(session: Session) {
        ...
        sessionStore[key] = Session(
            id = key,
            performance = session.getPerformance(),
            seats = session.getSeats().map { seat ->
                Seat(
